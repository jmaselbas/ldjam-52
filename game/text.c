...
void
sys_text_push(struct text_entry *entry)
{
	struct system *sys = &gs->sys_text;
...
void
sys_text_puts(vec2 at, vec3 color, char *s)
{
	float width = gs->width;
	float height = gs->height;
	size_t len = strlen(s);
	float size = 64;
	vec3 txt_scale = {size/width, size/height, 1.0};

	sys_text_push(&(struct text_entry) {
			.text = s,
			.len = len,
...
void
sys_text_printf(vec2 at, vec3 color, char *fmt, ...)
{
	char buf[512];
	va_list ap;
	va_start(ap, fmt);
...
sys_text_exec(void)
{
	...
	struct game_asset *game_asset = ga;
